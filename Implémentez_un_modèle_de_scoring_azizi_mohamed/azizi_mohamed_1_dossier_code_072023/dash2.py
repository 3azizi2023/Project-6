...
import joblib
import shap
from sklearn.cluster import KMeans  # Importez KMeans depuis sklearn.cluster
...
# Fonction pour afficher la capacité sous forme de graphique à barres
...
if selected_client_id:
    ...
    # Obtenir une prédiction directement à partir du modèle
...
# Fonction pour obtenir des clients similaires en utilisant K-Means
def get_similar_clients_kmeans(selected_client_id, num_similar_clients=5):
    if selected_client_id not in X_test['SK_ID_CURR'].values:
        st.error("ID du client sélectionné non trouvé dans les données de test.")
        return []

    # Obtenir les données du client sélectionné
    selected_client_data = X_test[X_test['SK_ID_CURR'] == selected_client_id]

    # Supprimer la colonne 'SK_ID_CURR' pour l'analyse de similarité
    selected_client_data = selected_client_data.drop(columns=['SK_ID_CURR'])

    # Créez un objet KMeans avec le nombre de clusters souhaité
    num_clusters = 100  # Vous pouvez ajuster ce nombre en fonction de vos besoins
    kmeans = KMeans(n_clusters=num_clusters, random_state=0)

    # Ajoutez les données de tous les clients à l'objet KMeans (sauf le client sélectionné)
    all_client_data = X_test.drop(columns=['SK_ID_CURR'])
    kmeans.fit(all_client_data)

    # Prédire le cluster du client sélectionné
    selected_client_cluster = kmeans.predict(selected_client_data)[0]

    # Trouvez les clients dans le même cluster que le client sélectionné
    similar_client_indices = np.where(kmeans.labels_ == selected_client_cluster)[0]

    # Obtenir les SK_ID_CURR des clients similaires
    similar_client_ids = X_test.iloc[similar_client_indices]['SK_ID_CURR'].values.tolist()
...
if st.button("Afficher quelques exemples de clients similaires"):
    similar_clients = get_similar_clients_kmeans(selected_client_id)

    if similar_clients:
        # Créer une liste des 10 premiers SK_ID_CURR des clients similaires
        similar_client_ids = similar_clients[:5]
